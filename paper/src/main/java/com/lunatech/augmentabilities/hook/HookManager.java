...
public class HookManager implements Reloadable {
    ...
    public HookManager(AugmentAbilities plugin) {
        this.plugin = plugin;
    }
    ...
    public void onLoad(AbstractAugmentAbilities plugin) {
        for (Hook hook : Hook.values()) {
            try {
                ...
                final AbstractHook hookInstance = hook.getHookClass().getDeclaredConstructor(AugmentAbilities.class).newInstance(plugin);
